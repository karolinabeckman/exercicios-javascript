...
const arrayEstados = [];
for (i = 1; i < linhasEstados.length; i++) {
	const estado = linhasEstados[i].split(";");
	const objetoEstado = {
		sigla: estado[0],
		nome: estado[1],
		capital: estado[2],
	...
	arrayEstados.push(objetoEstado);
}
...
function buscaPorRegiao(estados, regiaoDesejada) {
	const estadosFiltradosPorRegiao = estados.filter(
		(estado) => estado.regiao === regiaoDesejada
	);
	return estadosFiltradosPorRegiao;
}
...
function buscaPorSigla(estados, siglaDesejada) {
	const estadosFiltradosPorUF = estados.filter(
		(estado) => estado.sigla === siglaDesejada
	);
	return estadosFiltradosPorUF;
}
